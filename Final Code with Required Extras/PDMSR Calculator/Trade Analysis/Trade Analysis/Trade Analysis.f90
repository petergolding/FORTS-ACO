...
!****************************************************************************
    ...
    implicit none
    real, dimension (140) :: c_index, e_index,i_index
    character, dimension(140):: c_names, e_names, i_names
    real,dimension (140,2) :: L_cap
    ...
    integer, dimension (140,2) :: storage


    open(unit=222,file="input_trade_data.txt",status="unknown")
!    open(unit=333,file="capital_locations.txt",status="unknown")
...
!    
!    
!    !read in capital locations
!    do i = 1,n_countries
!        read(333,*)(L_cap(i,j),j=1,2)
!        do j = 1,2
!            L_cap(i,j)=L_cap(i,j)*3.14159265358979323846/180
!        end do
!    end do
!    
!    close(333)
...
!            a=(sin((exp_location(1,1)-imp_location(1,1))/2))**2.0+cos(imp_location(1,1))*cos(exp_location(1,1))*(sin((exp_location(1,2)-imp_location(1,2))/2))**2.0
!            c=2*atan2(sqrt(a),sqrt(1-a))
!            distance = 6371*c   !radius of the earth = 6371km
!            num_carriers(v_count)= trade_value(v_count)/100000 +1      !100000 is the min_ship_size
!            distance_cost(v_count) = distance_cost(i)+distance*num_carriers(v_count)
...
!    !calculates total distance cost
!    do j = 1,v_count
!        sum_distance_cost=sum_distance_cost + distance_cost(j)
!    end do
!    
    ...
    do i = 1,n_countries
        if (storage(i,2) < 0) then
            deficit_cost = deficit_cost + abs(storage(i,2))
        end if
    end do
    ...
    do i = 1,n_countries
        ...
        write(777,"(I6,',',I20)")storage(i,1),storage(i,2)
    end do
